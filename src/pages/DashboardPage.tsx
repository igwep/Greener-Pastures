...
export function DashboardPage() {
  ...
  console.log("[DashboardPage] dashboard_summary:", summary);
  console.log("[DashboardPage] isOnline:", isOnlineStatus);
  ...
  const handleDepositSubmit = async () => {
    if (!depositAmount || !depositScreenshotFile) {
      alert("Please enter amount and upload proof screenshot");
      return;
    }
          ...
          {isLoading ? (
            ...
            <Card className="animated-border flex flex-col p-6 rounded-3xl border-none shadow-sm overflow-visible bg-white">
              ...
              <div className="space-y-6 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                ...
                {/* Group: Today */}
                {recentActivity.filter((t: any) => t.group === "Today").length >
                  0 && (
                  <div>
                    ...
                    <div className="space-y-4">
                      {recentActivity
                        .filter((t: any) => t.group === "Today")
                        .map((tx: any) => (
                          <div
                            key={tx.id}
                            className={`flex items-center gap-4 group ${tx.isLoanRepayment ? 'opacity-75' : ''}`}
                          >
                            <div
                ...
                {/* Group: Yesterday */}
                {recentActivity.filter((t: any) => t.group === "Yesterday")
                  .length > 0 && (
                  <div>
                    <h3 className="text-xs font-bold uppercase tracking-widest text-ink-muted mb-4">
                      Yesterday
                    ...
                    <div className="space-y-4">
                      {recentActivity
                        .filter((t: any) => t.group === "Yesterday")
                        .map((tx: any) => (
                          <div
                            key={tx.id}
                            className={`flex items-center gap-4 group ${tx.isLoanRepayment ? 'opacity-75' : ''}`}
